...
namespace embree
{
  ...
  static size_t rays_per_frame = 0;
  ...
  static void parseCommandLine(Ref<ParseStream> cin, const FileName& path)
  {
	...
	g_interactive = false;
      ...
      /* set number of rays per frame */
      else if (tag == "-rays-per-frame")
        rays_per_frame = cin->getInt();
  ...
  void renderBenchmark(const FileName& fileName)
  {
    ...
    for (size_t i=0; i<numTotalFrames; i++) 
    {
      ...
      if (rays_per_frame) std::cout << 1E-6*float(rays_per_frame)/(t1-t0) << "Mrps ";
    ...
    std::cout << double(g_numBenchmarkFrames)/dt << "fps ";
    if (rays_per_frame) std::cout << 1E-6*double(g_numBenchmarkFrames)*float(rays_per_frame)/dt << "Mrps ";
    std::cout << std::endl;
    std::cout << "BENCHMARK_RENDER " << double(g_numBenchmarkFrames)/dt << std::endl;
    if (rays_per_frame) std::cout << "BENCHMARK_RENDER_MRPS " << 1E-6*double(g_numBenchmarkFrames)*float(rays_per_frame)/dt << std::endl;
  }
